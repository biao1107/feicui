<template>
  <div class="page account">
    ...
    <button class="cta-btn" @click="onContact">{{ auth.isVip ? '联系运营续费' : '联系运营升级' }}</button>
  </div>
</template>
...
import { getProfile, getDashboard } from '@/api/merchant'
...
.member-card { margin: 12px; border-radius: 14px; padding: 24px 20px; background: linear-gradient(135deg, #e8edf2, #f5f7fa); color: var(--color-text-secondary); }
...
.table { background: #fff; margin: 0 12px 12px; border-radius: 12px; padding: 16px; }
...
.cta-btn { margin: 0 16px calc(16px + env(safe-area-inset-bottom)); height: 48px; border: none; border-radius: 10px; background: var(--color-primary); color: #fff; font-size: 16px; }
</style>
